...
function TodoList({tasks}) {
    ...
    return (
      <section>
      {tasks.map((task) =>   
      <div className={"mx-auto h-full p-4 mt-4 w-4/5 dark:bg-gray-800 shadow-lg dark:border-gray-700 border-2 flex justify-center items-center slide-in-bottom md:w-2/5" + (task.status ? " border-blue-400 dark:border-blue-400 border-2":"")} key={task.id}>       
          ...
          className={"text-gray-600 w-6 h-6 flex-shrink-0 mr-4 hover:text-blue-400" + (task.status ? " text-blue-400 ":"")} onClick={ () => completeTask(task.id)} >
